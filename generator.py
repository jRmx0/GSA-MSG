...
class ToneGenerator(QWidget):
    ...
    def generate_tone(self, wave_type="sine", frequency=440, duration=1.0, amplitude=0.5):
        """Generate a monotone waveform (normalized -1.0 to 1.0)"""
    ...
    def on_generate_clicked(self):
        ...
        try:
            ...
            # Stop any ongoing playback before regenerating.
            self.stop_playback()

            t, wave = self.generate_tone(wave_type, freq, duration, amp)
    ...
    def render_waveform(self, wave_type, freq):
        ...
        if self.wave_time is None or self.wave_int16 is None:
            return

        self.figure.clear()
        ax = self.figure.add_subplot(111)
        ax.plot(self.wave_time, self.wave_int16, linewidth=0.8)
        ax.set_title(f"{wave_type.capitalize()} Wave – {freq} Hz ({self.sample_rate/1000:.1f} kHz, 16-bit)")
        ax.set_xlabel("Time (s)")
        ax.set_ylabel("Amplitude (16-bit integer)")
        ax.grid(True)

        ax.yaxis.set_major_formatter(FormatStrFormatter('%d'))
        max_val = np.max(np.abs(self.wave_int16)) if self.wave_int16.size else 0
        if max_val == 0:
            max_val = 32767
